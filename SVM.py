...
class SVM:
    ...
    def define_dataframe(self):
        ...
        self.training_feature.to_excel("SVM_training_feature_all.xlsx")
        self.training.to_excel("SVM_training_all.xlsx")
        self.testing_feature.to_excel("SVM_testing_feature_all.xlsx")
        self.test.to_excel("SVM_test_all.xlsx")
    ...
    def main(self):
        self.svclassifier = SVC(kernel='linear', C=100000)
        self.svclassifier.fit(self.training_feature, self.training['result'])
        self.accuracy(self.training_feature, self.training['result'], self.testing_feature, self.test['result'])

    def accuracy(self, data_training, target_training, data_test, target_test):
        # check results

        predict_test = self.svclassifier.predict(data_test)

        print(" 1.1- f1 score for GP: ")
        print(f1_score(target_test, predict_test, average='macro'))
        print(" 1.2- f1 score for GP None: ")
        print(f1_score(target_test, predict_test, average=None))

        # Calculate the accuracy
        print("2 - accuracy score for GP: ")
        print(accuracy_score(target_test, predict_test, normalize=True))

        # KFold Cross Validation approach
        kf = KFold(n_splits=10, shuffle=False)
        kf.split(data_training)
        ...
        for train_index, test_index in kf.split(data_training):
            # Split train-test
            X_train, X_test = data_training.iloc[train_index], data_training.iloc[test_index]
            y_train, y_test = target_training.iloc[train_index], target_training.iloc[test_index]
            # Train the model
            self.svclassifier.fit(X_train, y_train)
            # Append to accuracy_model the accuracy of the model
            acc = accuracy_score(y_test, self.svclassifier.predict(X_test), normalize=True) * 100
    ...
    def read_xlsx(self):
        self.training_feature = pd.read_excel("SVM_training_feature_all.xlsx")
        self.training = pd.read_excel("SVM_training_all.xlsx")
        self.testing_feature = pd.read_excel("SVM_testing_feature_all.xlsx")
        self.test = pd.read_excel("SVM_test_all.xlsx")


        # training_feature
        dataframe = self.training_feature
        ...
        self.training_feature = self.training_feature.iloc[:, fit.support_]
        self.testing_feature = self.testing_feature.iloc[:, fit.support_]
        self.main()
